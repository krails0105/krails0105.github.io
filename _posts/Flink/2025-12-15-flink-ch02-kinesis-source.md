...
date: 2026-03-13
categories: [Flink]
tags: [Flink, Kinesis, Java, FlinkKinesisConsumer, DeserializationSchema]
---

### 들어가며

Ch01에서 `DataGen` Source로 가상 데이터를 생성했다면, Ch02에서는 실제 AWS Kinesis 스트림에 연결해 암호화폐 시계열 집계(Time-Series Aggregation) 캔들 데이터를 읽는다. Java와 Maven을 처음 다루면서 `FlinkKinesisConsumer`, `DeserializationSchema`, Lombok까지 한꺼번에 다뤄야 하는 챕터였다.
...
실습 환경에는 이미 운영 중인 Kinesis 스트림(`data-stream-tsagg`)이 있다. 이 스트림에는 데이터 소스에서 수집한 시계열 집계 데이터가 JSON 형태로 계속 들어온다. 목표는 Flink 잡이 이 스트림을 읽어서 콘솔에 출력하는 것.
...
```java
// src/main/java/com/study/model/TSAgg.java
...
public class TSAgg implements Serializable { // Flink이 네트워크로 객체 전송 시 직렬화 필요
...
public class Ch02_KinesisSource {
    ...
    public static void main(String[] args) throws Exception {
        ...
        FlinkKinesisConsumer<TSAgg> consumer = new FlinkKinesisConsumer<>(
                config.getProperty("stream.name"),  // 스트림 이름
                new TSAggDeserializer(),             // byte[] → TSAgg 변환기
                kinesisProps
        );

        // 5. Source 등록 → 콘솔 출력
        env.addSource(consumer, "Kinesis TSAgg")
                .print();  // TSAgg.toString() 호출 — Lombok @Data가 자동 생성
...
#### 5. TSAggDeserializer (inner class)
...
static class TSAggDeserializer implements DeserializationSchema<TSAgg> {
    ...
    @Override
    public TSAgg deserialize(byte[] bytes) throws IOException {
        if (mapper == null) {
            mapper = new ObjectMapper();
        }
        return mapper.readValue(bytes, TSAgg.class);
    }

    @Override
    public boolean isEndOfStream(TSAgg tsAgg) {
        return false;  // 무한 스트림 — 끝이 없다
    }

    @Override
    public TypeInformation<TSAgg> getProducedType() {
        return TypeInformation.of(TSAgg.class);  // null 리턴하면 런타임 에러
    }
}
```
...
**왜 `transient`인가?** Flink은 이 `TSAggDeserializer` 인스턴스를 Java 직렬화로 TaskManager에 전송한다. Jackson의 `ObjectMapper`는 `Serializable`을 구현하지 않기 때문에 `transient`로 선언하지 않으면 `NotSerializableException`이 발생한다. 대신 `deserialize()` 메서드 첫 호출 시 lazy initialization으로 생성한다.
...
```text
TSAgg(symbol=BTC/USDT, base=BTC, quote=USDT, marketType=spot, timestamp=1710300000000, open=67234.5, high=67300.0, low=67100.2, close=67250.8, volume=123.456)
```
...
처음에 `return null`로 두었다가 런타임에 Flink의 타입 시스템이 `NullPointerException`을 냈다. `TypeInformation.of(TSAgg.class)`를 반드시 리턴해야 한다. Flink은 이 타입 정보를 기반으로 직렬화/역직렬화 전략과 메모리 관리를 결정하기 때문이다.
