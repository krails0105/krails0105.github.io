...
`PipelineMonitorJob.java`에서 소스 생성 코드가 달라진다. 핵심 차이는 두 가지다: (1) `setStreamArn()`으로 스트림을 지정하고, (2) Kafka 전용 `KafkaRecordDeserializationSchema` 대신 범용 `DeserializationSchema`를 사용한다.

```java
// flink-job/src/main/java/com/tech/monitor/PipelineMonitorJob.java

// Before: KafkaSource
KafkaSource<PipelineEvent> kafkaSource = KafkaSource.<PipelineEvent>builder()
    .setBootstrapServers("kafka:9092")
    .setTopics("pipeline-events")
    .setDeserializer(KafkaRecordDeserializationSchema.of(new PipelineEventDeserializer()))
    .build();
DataStreamSource<PipelineEvent> events = env.fromSource(kafkaSource, watermark, "Kafka");

// After: KinesisStreamsSource
KinesisStreamsSource<PipelineEvent> kinesisSource = KinesisStreamsSource
    .<PipelineEvent>builder()
    .setStreamArn(env("KINESIS_STREAM_ARN", "arn:aws:kinesis:us-west-1:ACCOUNT_ID:stream/pipeline-events"))
    .setDeserializationSchema(new PipelineEventDeserializer())
    .build();

SingleOutputStreamOperator<PipelineEvent> events = env.fromSource(kinesisSource, watermark, "Kinesis Pipeline Events")
    .returns(PipelineEvent.class);  // ← 이게 없으면 나중에 type erasure 에러 발생 (에러 7)
```

`.returns(PipelineEvent.class)` 한 줄이 중요하다. Flink는 스트림의 타입 정보를 런타임에 유지해야 직렬화를 처리할 수 있는데, Java 컴파일러의 type erasure 때문에 `KinesisStreamsSource<PipelineEvent>`의 타입 파라미터가 지워진다. `.returns()`로 명시하지 않으면 배포 단계에서 에러 7번으로 다시 만나게 된다.

### PipelineEventDeserializer 변경
...
```java
// flink-job/src/main/java/com/tech/monitor/PipelineEventDeserializer.java

// Before
public class PipelineEventDeserializer implements KafkaRecordDeserializationSchema<PipelineEvent> { ... }

// After
public class PipelineEventDeserializer extends AbstractDeserializationSchema<PipelineEvent> {
    private static final ObjectMapper mapper = new ObjectMapper();

    @Override
    public PipelineEvent deserialize(byte[] bytes) throws IOException {
        return mapper.readValue(bytes, PipelineEvent.class);
    }
}
```
...
// 사용 예시
pgProps.setProperty("url", env("JDBC_URL", "jdbc:postgresql://localhost:5432/monitoring"));
...
kubectl describe flinkdeployment pipeline-monitor -n monitoring
```
...
COPY flink-job/target/flink-job-1.0.0.jar /opt/flink/usrlib/flink-job.jar
```

```bash
# 1. Fat JAR 빌드
mvn -pl flink-job package -DskipTests

# 2. Docker 이미지 빌드 (EKS 노드가 amd64이므로 플랫폼 명시 필수)
docker build --platform linux/amd64 -t tech-pipeline-monitor:latest .
...
# 4. 태그 & 푸시
docker tag tech-pipeline-monitor:latest \
  ACCOUNT_ID.dkr.ecr.us-west-1.amazonaws.com/tech-pipeline-monitor:latest

docker push ACCOUNT_ID.dkr.ecr.us-west-1.amazonaws.com/tech-pipeline-monitor:latest
```
...
kubectl apply -f infra/flink/flink-deployment.yaml
```
...
**원인**: Flink는 스트림의 타입 정보를 런타임에 유지해야 직렬화/역직렬화를 처리할 수 있다. Java 컴파일러의 type erasure로 `KinesisStreamsSource<PipelineEvent>`의 타입 파라미터가 지워지면서, Flink가 런타임에 타입을 추론하지 못한다. 이 문제는 Flink 공식 문서의 [Java Lambda Expressions](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/datastream/java_lambdas/) 페이지에서도 다루고 있다.

**해결**: `.returns()`로 타입을 명시한다. 단순 POJO라면 `.returns(PipelineEvent.class)`로 충분하고, 제네릭 타입이라면 `.returns(new TypeHint<Tuple2<Integer, SomeType>>(){})` 형태를 쓴다.

```java
// flink-job/src/main/java/com/tech/monitor/PipelineMonitorJob.java

SingleOutputStreamOperator<PipelineEvent> events =
    env.fromSource(kinesisSource, watermarkStrategy, "Kinesis Pipeline Events")
       .returns(PipelineEvent.class);  // ← 이 한 줄이 핵심
```
...
# infra/flink/flink-deployment.yaml
spec:
  image: ACCOUNT_ID.dkr.ecr.us-west-1.amazonaws.com/tech-pipeline-monitor:latest
...
```
AccessDeniedException: User: arn:aws:sts::ACCOUNT_ID:assumed-role/KarpenterNodeRole/...
...
**1) `PipelineMonitorJob.java`의 `thresholds` 맵** -- static 필드라 직렬화 대상이 아니므로 문제없음:
```java
// 이건 static 필드라 직렬화 대상 아님 — 문제 없음
static Map<String, Long> thresholds = Map.ofEntries(...);
```

**2) `FreshnessChecker`와 `ErrorRateChecker`에서 Alert 생성 시** -- Flink 오퍼레이터 내부에서 만들어진 객체는 직렬화 대상:
```java
...
**3) `PipelineEvent`와 `Alert`의 필드 타입 변경** -- `Object` 타입은 Kryo가 런타임에 타입을 추론해야 해서 불안정:
```java
...
event = {
    "domain": "offchain",
    "source": "bigquery",
    "source_id": "offchain_data_bronze.bitcoin_etf",
    "stage": "error",
    "event_type": "ERROR",
...
response = kinesis.put_record(
    StreamName="pipeline-events",
    Data=json.dumps(event),
    PartitionKey="offchain_data_bronze.bitcoin_etf"
)
...
---------------------------------------------|----------|---------|----------------------------
offchain_data_bronze.bitcoin_etf.error.error | CRITICAL | FIRING  | 2026-03-29 10:00:05.123+00
```
...
```yaml
# infra/flink/flink-deployment.yaml (민감 정보 치환됨)
...
spec:
        ...
        - name: flink-main-container
          env:
            - name: KINESIS_STREAM_ARN
              value: "arn:aws:kinesis:us-west-1:ACCOUNT_ID:stream/pipeline-events"
            - name: JDBC_URL
              value: "jdbc:postgresql://DB_HOST:5432/monitoring"
            - name: JDBC_USERNAME
              value: "monitor"
            - name: JDBC_PASSWORD
              value: "CHANGE_ME"       # Secret으로 분리 예정
            - name: SLACK_ENABLED
              value: "false"
  ...
  job:
    ...
    entryClass: com.tech.monitor.PipelineMonitorJob
    parallelism: 1
...
- `flink-job/src/main/java/com/tech/monitor/PipelineMonitorJob.java`
- `flink-job/src/main/java/com/tech/monitor/PipelineEventDeserializer.java`
- `flink-job/src/main/java/com/tech/monitor/model/PipelineEvent.java`
- `flink-job/src/main/java/com/tech/monitor/model/Alert.java`
- `flink-job/src/main/java/com/tech/monitor/function/FreshnessChecker.java`
- `flink-job/src/main/java/com/tech/monitor/function/ErrorRateChecker.java`
- `flink-job/pom.xml`
- `infra/flink/flink-deployment.yaml`
- `Dockerfile`
