...
내부 발행 메서드에서 모든 예외를 `try/except`로 감싸고, 실패는 로그만 남긴 채 무시한다. **파이프라인이 죽는 것보다 모니터링 이벤트 하나를 놓치는 게 낫다.** 이 원칙이 모든 설계의 출발점이다.

### 2. Context Manager (`track()`)

`start()` -> 작업 수행 -> `complete()` / `error()` 패턴은 반복 코드가 많아진다. `track()` context manager로 이 패턴을 캡슐화했다.

수동으로 `start → complete/error`를 호출하는 대신, `track()` context manager로 감싸면 START/COMPLETE/ERROR + elapsed 계산이 자동화된다. 예외 발생 시 ERROR 발행 후 반드시 re-raise하여 본체 로직에 영향을 주지 않는다.
...
```python
mp = MonitoringProducer(source_id="my_pipeline")

mp.start("extract")
mp.complete("extract", elapsed_ms=1200)
mp.error("extract")

with mp.track("transform"):  # START → 작업 → COMPLETE or ERROR
    do_work()
```

4개의 메서드가 전부다. `track()`은 context manager로 START/COMPLETE/ERROR + elapsed 계산을 자동화한다.

### 설계 포인트

| 원칙 | 구현 방법 |
|---|---|
| Silent Fail | 내부 발행 메서드에서 모든 예외를 catch → 로그만 남기고 무시 |
| 본체 예외 보존 | `track()` 내부에서 ERROR 발행 후 반드시 re-raise |
| 브로커 추상화 | 외부 API 4개는 고정, 내부 발행 메서드만 교체하면 Kafka↔Kinesis 전환 가능 |
| PartitionKey | `source_id.stage` 형식 → 같은 파이프라인 이벤트가 같은 샤드로 라우팅 |
...
변경된 것은 내부 발행 메서드 하나뿐이다.

| | Kafka | Kinesis |
|---|---|---|
| 클라이언트 | `confluent_kafka.Producer` | `boto3.client("kinesis")` |
| 발행 API | `.produce(topic, key, value)` | `.put_record(StreamName, Data, PartitionKey)` |
| key 타입 | bytes (`.encode()` 필요) | string |
| flush | 명시적 `.flush()` 필요 | 불필요 (동기 호출) |
| 의존성 | `confluent-kafka` | `boto3` (이미 존재) |

외부 API(`start`, `complete`, `error`, `track`)는 변경 없음. **브로커 의존성을 하나의 메서드 뒤에 숨겨둔 덕분에** 전환이 이 정도로 끝났다.
