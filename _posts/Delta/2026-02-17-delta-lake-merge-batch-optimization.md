---
title: "[Delta] Delta Lake MERGE 성능 최적화: 수십억 Rows 테이블 증분 업데이트 전략"
categories:
  - Delta
tags: [Delta Lake, Spark, MERGE, Performance Optimization, Databricks, Liquid Clustering, AQE]
---
...
대규모 트랜잭션 데이터를 처리하는 과정에서 **~수십억 rows, 수백 GB 크기의 `transaction_base` 테이블에 대한 증분 업데이트**가 극도로 느린 문제를 겪었습니다. 소비된 트랜잭션 정보를 UPDATE하는 단일 MERGE 쿼리가 수백 GB 전체를 shuffle하며 실질적으로 완료되지 않는 상황이었고, Spot Instance의 worker 회수 장애까지 겹쳐 작업이 불가능했습니다.
...
대규모 트랜잭션 원장에서 모든 미처리 트랜잭션 데이터를 추적하는 `transaction_base` 테이블입니다:

```sql
-- transaction_base 스키마
-- Liquid Clustering: CLUSTER BY (created_event_id, consumed_event_id)
created_event_id INT              -- 트랜잭션 생성 이벤트 (0~936K)
created_transaction_hash STRING   -- 생성 트랜잭션 해시
created_output_index INT          -- 트랜잭션 내 출력 인덱스
value DECIMAL(16,8)               -- 금액
created_price DECIMAL(16,8)       -- 생성 시점 가격 (USD)
consumed_event_id INT             -- 소비 이벤트 (NULL = 미소비)
spent_transaction_hash STRING     -- 소비 트랜잭션 해시
spent_price DECIMAL(16,8)         -- 소비 시점 가격 (USD)
...
```

- **크기**: ~수십억 rows, 수백 GB (Parquet)
- **Liquid Clustering**: `created_event_id`, `consumed_event_id` 기준 클러스터링
- **증분 업데이트 시나리오**: 신규 이벤트(935180~936369)에서 소비된 트랜잭션의 `spent_*` 컬럼 UPDATE

### 문제 상황

#### 1. 단일 MERGE의 실패

소비된 트랜잭션은 전체 히스토리에 분산되어 있습니다. 최근 이벤트(935180~936369)에서 소비된 트랜잭션의 `created_event_id`는 0부터 936K까지 넓게 퍼져 있기 때문에, MERGE의 타겟 스캔이 테이블 전체를 읽어야 합니다.

- 소스 데이터: 6.74M rows, 617MB
- 타겟 스캔: 수백 GB 전체 (Liquid Clustering data skipping 미작동)
- AQE가 **SortMergeJoin으로 전환** --> 양쪽 모두 shuffle --> 수백 GB 네트워크 이동
...
- **SortMergeJoin**: 양쪽 데이터가 모두 클 때 선택됩니다. **양쪽 모두를 join key로 shuffle(재분배)한 후 정렬하여 머지**합니다. 수백 GB 테이블의 shuffle은 네트워크 I/O와 디스크 I/O 비용이 막대합니다.
...
1. **100K `created_event_id` 범위로 배치 분할** (0-99999, 100000-199999, ...)
2. 배치당 소스가 수백~수만 rows로 줄어듦 --> AQE가 BroadcastHashJoin 선택
3. ON 절에 `t.created_event_id BETWEEN batch_start AND batch_end` 추가 --> PushedFilters 전달 --> 해당 범위 파일만 읽음
...
- **타겟 범위 축소** --> PushedFilters로 data skipping 작동 (수백 GB --> 수 GB)
...
```sql
MERGE INTO transaction_base t
USING (
    SELECT ... FROM inputs
    WHERE event_id BETWEEN 935180 AND 936369
) s
ON t.created_event_id = s.created_event_id
...
- 타겟 스캔: **PushedFilters 없음** --> 수백 GB 전체 스캔
- 문제: ON 절에 타겟의 범위 조건이 없으므로 data skipping이 전혀 작동하지 않음

#### Plan 2: 범위 필터 추가 (역효과 발생)

ON 절에 소스의 실제 `created_event_id` 범위를 추가해봤습니다:

```sql
ON t.created_event_id = s.created_event_id
   AND t.created_transaction_hash = s.created_transaction_hash
   AND t.created_output_index = s.created_output_index
   AND t.created_event_id BETWEEN 73650 AND 936236  -- 소스의 min/max 범위
```

**Query Plan 결과**:
- 타겟 스캔: PushedFilters 반영됨
  ```
  PushedFilters: [IsNotNull(created_event_id),
                  (created_event_id >= 73650),
                  (created_event_id <= 936236)]
  ```
- **BUT**: 소스 617MB가 AQE broadcast 임계값(30MB)을 초과 --> AQE가 **SortMergeJoin으로 전환** --> 수백 GB shuffle 발생
...
```sql
-- 예시 배치: created_event_id 173650 ~ 273649
MERGE INTO transaction_base t
USING (
    SELECT ... FROM inputs
    WHERE event_id BETWEEN 935180 AND 936369
      AND prevout_event_id BETWEEN 173650 AND 273649  -- 배치 범위로 소스 축소
) s
ON t.created_event_id = s.created_event_id
   AND t.created_transaction_hash = s.created_transaction_hash
   AND t.created_output_index = s.created_output_index
   AND t.created_event_id BETWEEN 173650 AND 273649   -- 타겟 PushedFilters 전달
WHEN MATCHED THEN UPDATE SET ...
```

**Query Plan 결과**:
- 소스: **564 rows, 52.9KB** --> AQE broadcast 임계값(30MB) 대비 극히 작음 --> **BroadcastHashJoin 확정**
- 타겟 스캔: PushedFilters `[created_event_id >= 173650, <= 273649]` --> **600개 중 4개 파일만 읽음**
- 실행 시간: **15.0초**
...
- 이벤트 ID 0~936K --> **10개 배치**로 분할 (관리 가능한 수)
...
BATCH_SIZE = 100_000
TRANSACTION_BASE = f"{catalog}.{schema}.transaction_base"
MAX_RETRIES = 3

# ============================================================
# Step 1: 신규 트랜잭션을 replaceWhere로 삽입 (idempotent)
# ============================================================
# replaceWhere는 파일 단위 atomic 교체이므로, 재실행해도 동일 결과를 보장합니다.
new_events_df = spark.sql(f"""
SELECT
    CAST(o.event_id AS INT) AS created_event_id,
    o.transaction_hash AS created_transaction_hash,
    CAST(o.output_index AS INT) AS created_output_index,
    o.value,
    p.price AS created_price,
    CAST(NULL AS INT) AS spent_event_id,
    CAST(NULL AS STRING) AS spent_transaction_hash,
    CAST(NULL AS DECIMAL(16,8)) AS spent_price
FROM transaction_output_df o
JOIN price_df p USING (event_id)
WHERE o.event_id BETWEEN {start_block} AND {end_block}
""")

new_events_df.write.format("delta") \
    .option("replaceWhere",
            f"created_event_id BETWEEN {start_block} AND {end_block}") \
    .mode("overwrite") \
    .saveAsTable(TRANSACTION_BASE)

# ============================================================
# Step 2: 사전 필터링 -- 데이터가 존재하는 배치만 추출
# ============================================================
# FLOOR(prevout_event_id / BATCH_SIZE)로 배치 ID를 계산하여,
# 실제 소비된 트랜잭션이 존재하는 배치만 식별합니다.
active_batches = spark.sql(f"""
    SELECT DISTINCT FLOOR(prevout_event_id / {BATCH_SIZE}) AS batch_id
    FROM transaction_input_df
    WHERE event_id BETWEEN {start_block} AND {end_block}
...
for row in active_batches:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            USING (
                SELECT
                    i.prevout_event_id AS created_event_id,
                    i.prevout_transaction_hash AS created_transaction_hash,
                    i.prevout_index AS created_output_index,
                    i.event_id AS spent_event_id,
                    i.transaction_hash AS spent_transaction_hash,
                    p_spent.price AS spent_price
                FROM transaction_input_df i
                JOIN price_df p_spent USING (event_id)
                WHERE i.event_id BETWEEN {start_block} AND {end_block}
                  AND i.prevout_event_id BETWEEN {batch_start_h} AND {batch_end_h}
                  AND i.is_coinbase = false
            ) s
            ON t.created_event_id = s.created_event_id
               AND t.created_transaction_hash = s.created_transaction_hash
               AND t.created_output_index = s.created_output_index
               AND t.created_event_id BETWEEN {batch_start_h} AND {batch_end_h}
            WHEN MATCHED THEN UPDATE SET
                t.spent_event_id = s.spent_event_id,
...
```sql
-- 이 조건이 없으면 PushedFilters가 전달되지 않아 수백 GB 전체를 스캔합니다
AND t.created_event_id BETWEEN {batch_start_h} AND {batch_end_h}
```
...
```sql
-- 소스 서브쿼리 WHERE 절
WHERE i.event_id BETWEEN {start_block} AND {end_block}       -- 신규 블록 범위
  AND i.prevout_event_id BETWEEN {batch_start_h} AND {batch_end_h}  -- 배치 범위
```
...
신규 트랜잭션을 `replaceWhere`로 먼저 삽입한 후 MERGE를 실행합니다. `replaceWhere`는 Delta Lake의 파일 단위 atomic 교체이므로 재실행해도 동일한 결과가 보장됩니다 (idempotent).
...
== Physical Plan ==
AdaptiveSparkPlan
+- BroadcastHashJoin [created_event_id, created_transaction_hash, created_output_index]
   :- Project ...
   |  +- FileScan parquet transaction_base
   |     PushedFilters: [IsNotNull(created_event_id),
   |                      (created_event_id >= 173650),
   |                      (created_event_id <= 273649)]
   |     ReadSchema: struct<...>
...
```
== Physical Plan ==
SortMergeJoin [created_event_id, ...]               <-- shuffle 기반 join
:- Sort ...
|  +- Exchange hashpartitioning                         <-- 수백 GB shuffle!
|     +- FileScan parquet transaction_base (수십억 rows)
+- Sort ...
...
```sql
-- [X] 이것은 소스만 필터링합니다. 타겟은 여전히 수백 GB 전체 스캔.
USING (
    SELECT ... FROM inputs
    WHERE prevout_event_id BETWEEN batch_start AND batch_end
) s
ON t.created_event_id = s.created_event_id ...

-- [O] 이것이 타겟의 PushedFilters로 전달됩니다.
ON t.created_event_id = s.created_event_id
   AND t.created_event_id BETWEEN batch_start AND batch_end
```
...
이번 케이스에서는 최근 이벤트에서 소비된 트랜잭션이 거의 모든 100K 범위에 분포하여, 사전 필터링으로 skip된 배치가 없었습니다.
...
### 6. 같은 이벤트 범위 내 생성-소비 트랜잭션 처리

같은 이벤트 범위(935180~936369)에서 생성되고 바로 소비되는 트랜잭션도 정확히 처리됩니다:

1. **Step 1 (replaceWhere)**: `spent_event_id = NULL`로 삽입 (아직 미소비 상태)
...
|---|---|---|---|---|
| 단일 MERGE (baseline) | 6.74M rows, 617MB | SortMergeJoin | 수백 GB 전체 | 완료 불가 (>2h 추정) |
| 단일 MERGE + 범위 필터 | 6.74M rows, 617MB | SortMergeJoin | ~수백 GB (범위가 거의 전체) | 더 느림 (shuffle 추가) |
...
- **비용**: SortMergeJoin의 수백 GB shuffle 제거 --> 네트워크 I/O 대폭 감소
...
batch_stats = spark.sql(f"""
    SELECT FLOOR(prevout_event_id / {BATCH_SIZE}) AS batch_id,
           COUNT(*) AS row_count
    FROM transaction_input_df
    WHERE event_id BETWEEN {start_block} AND {end_block}
      AND is_coinbase = false
...
```sql
-- Step 1: 영향받는 트랜잭션 삭제
DELETE FROM transaction_base
WHERE created_event_id IN (
    SELECT DISTINCT prevout_event_id
    FROM transaction_input_df
    WHERE event_id BETWEEN start_block AND end_block
);

-- Step 2: 업데이트된 트랜잭션 재삽입
INSERT INTO transaction_base
SELECT ... FROM ... JOIN ...;
```
...
- `base/legacy_model.py` (기존 Python RocksDB 기반 트랜잭션 처리)
