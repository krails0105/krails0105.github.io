...
categories:
  - ClaudeCode
tags: [Claude Code, Plugin, Skill, Troubleshooting, Slash Command]
---
...
- {mycompany}:crypto (플러그인)
- {mycompany}:crypto-whale (플러그인)
- {mycompany}:crypto-market (플러그인)
- {mycompany}:crypto-signal (플러그인)
```

반면 `{mycompany}:*` 스킬들은 Skill 도구로도 정상 호출되었다. **플러그인으로 등록된 스킬만 Skill 도구에서 인식된다**는 것이 힌트였다.
...
왜 `{mycompany}:*` 스킬들은 정상 동작하는지, 해당 플러그인의 등록 구조를 하나씩 확인했다.
...
```json
{
  "{mycompany}": {
    "source": {
      "source": "github",
      "owner": "{mycompany}-official",
      "repo": "claude-code-plugins"
    },
    "installLocation": "/Users/test/.claude/plugins/cache/{mycompany}"
  }
}
```
...
```json
{
  "claude-{mycompany}-plugin@{mycompany}": [
    {
      "scope": "user",
      "installPath": "/Users/test/.claude/plugins/cache/{mycompany}/claude-{mycompany}-plugin/0.1.0",
      "version": "0.1.0"
    }
  ]
}
...
{
  "enabledPlugins": {
    "claude-{mycompany}-plugin@{mycompany}": true
  }
}
```
...
`~/.claude/plugins/cache/{mycompany}/claude-{mycompany}-plugin/0.1.0/.claude-plugin/plugin.json`:

```json
{
  "name": "claude-{mycompany}-plugin",
  "version": "0.1.0",
  "description": "{MyCompany} Plugin for Claude Code",
  "skills": "./skills/"
}
```
...
```
~/.claude/plugins/cache/{mycompany}/claude-{mycompany}-plugin/0.1.0/
└── skills/
    ├── crypto/SKILL.md
