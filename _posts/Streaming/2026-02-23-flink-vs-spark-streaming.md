...
categories:
  - Streaming
tags: [Flink, Spark, Streaming, Comparison, Architecture]
---
...
Flink는 **Barrier Alignment**과 **Two-Phase Commit(2PC)** 두 가지 메커니즘으로 exactly-once를 보장합니다.

#### Barrier Alignment — "이 시점까지 처리 완료"의 경계 만들기

Flink는 스트림에 **체크포인트 배리어(barrier)**라는 특수 마커를 삽입합니다. 이 배리어가 모든 병렬 채널에서 도착할 때까지 기다린 뒤, 그 시점의 operator 상태를 스냅샷합니다.

```
Channel 1: ──데이터A──데이터B──|barrier|──데이터C──
Channel 2: ──데이터D──────────────|barrier|──데이터E──
                                    ↓
                        두 barrier가 모두 도착하면
                        → operator 상태 스냅샷 저장
                        → "barrier 이전 = 처리 완료 / 이후 = 미처리"
```

**왜 Alignment이 필요한가?** 병렬 채널의 처리 속도가 다르면, 빠른 채널의 barrier가 먼저 도착합니다. 이때 느린 채널의 barrier가 올 때까지 빠른 채널의 데이터를 **버퍼링**해야 정확한 스냅샷이 됩니다. 이 대기 과정이 Alignment입니다.

- Alignment 중 약간의 레이턴시가 발생할 수 있음
- Flink 1.11+에서는 `Unaligned Checkpoint` 옵션으로 대기 없이 처리 가능 (backpressure 상황에서 유리)

#### Two-Phase Commit (2PC) — Sink까지 exactly-once 보장

Barrier Alignment은 **Flink 내부 상태**의 정확성만 보장합니다. 외부 시스템(DB, 파일 등)에 쓰는 **Sink**까지 exactly-once를 보장하려면 2PC가 필요합니다.

```
Phase 1 (Pre-commit):
  → 체크포인트 진행 중 — 데이터를 임시 영역에 쓰기 (확정 전)
  → 예: Kafka 트랜잭션 시작, 임시 파일 생성

Phase 2 (Commit):
  → 체크포인트 성공 확인 → 임시 데이터를 확정
  → 예: Kafka 트랜잭션 커밋, 임시 파일을 최종 위치로 이동

Rollback:
  → 체크포인트 실패 → 임시 데이터 삭제
  → 예: Kafka 트랜잭션 abort, 임시 파일 삭제
```

**Sink가 2PC를 지원해야** 이 방식이 동작합니다. Kafka, HDFS/S3(파일)는 지원하지만, 일반 JDBC Sink는 지원하지 않아 at-least-once + idempotent write로 대체합니다.

| Sink | 2PC 지원 | Exactly-once 방식 |
|------|:---:|------|
| Kafka | O | 트랜잭션 기반 |
| S3/HDFS (파일) | O | 임시 파일 → rename |
| JDBC (PostgreSQL 등) | X | at-least-once + UPSERT |
