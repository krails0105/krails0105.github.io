...
categories:
  - Spark
tags: [Spark, Partition, SparkUI, Skew, Spill, AQE]
---
...
50노드로 스케일업 + repartition(800) 적용 후:

| 항목 | Before (repartition 없음) | After (repartition 800) |
|------|:---:|:---:|
| 인스턴스 | 클러스터 인스턴스 × 30 | 클러스터 인스턴스 × 50 |
| 220K blocks | **18시간** | - |
| 190K blocks | - | **1시간 11분** |
...
|----------|:---:|:---:|:---:|
| 클러스터 인스턴스(소) × 20 | 20 | ~136 | 200~400 |
| 클러스터 인스턴스(소) × 50 | 50 | ~340 | **800** |
| 클러스터 인스턴스(중) × 20 | 20 | ~280 | 600 |
...
3. Autoscaling Thrashing (스케일다운 ↔ 업 반복)
   → 고정 클러스터 사용 (min = max workers)
   → 아래 "Autoscaling 함정" 섹션 참조
...
## 8. Autoscaling 함정: 배치 파이프라인에서의 Thrashing

---

### 증상: 2분 간격 리사이징 반복

repartition 적용 후 50노드로 실행했는데, 클러스터 이벤트 로그에서 이런 패턴이 관찰되었습니다:

```text
23:52:41  UPSIZE_COMPLETED   50개 노드로 업사이징 완료
23:51:52  RESIZING           35 → 50 자동 크기 조정
23:51:21  RESIZING           50 → 35 자동 크기 조정
23:50:46  UPSIZE_COMPLETED   50개 노드로 업사이징 완료
23:49:52  RESIZING           34 → 50 자동 크기 조정
23:48:52  RESIZING           50 → 34 자동 크기 조정
...2분 간격으로 무한 반복
```

### 원인

배치 파이프라인은 배치 간 짧은 유휴 구간이 있습니다 (캐시 해제 → 다음 뷰 준비):

```text
[배치 1 실행 ████████████] [유휴] [배치 2 실행 ████████████] [유휴] ...
                           ↑ autoscaler: "유휴 감지 → 스케일다운!"
                                  ↑ 다음 배치 시작 → "리소스 부족 → 스케일업!"
```

### 스케일다운이 위험한 이유

```text
1. 제거된 노드의 DISK_ONLY 캐시 유실 → 재계산 비용
2. 스케일업에 1~2분 소요 → 그 동안 코어 부족으로 성능 저하
3. 새 executor 워밍업 오버헤드
4. 리사이징 자체가 클러스터 안정성에 영향
```

### 해결: 고정 클러스터

장시간 실행하는 배치 파이프라인에서는 **autoscaling OFF**:
...
### Autoscaling이 적합한 경우 vs 아닌 경우

| 워크로드 | Autoscaling | 이유 |
|----------|:-----------:|------|
| Ad-hoc 쿼리/노트북 | O | 유휴 시간 길고 불규칙 |
| 스트리밍 | O | 트래픽 변동에 대응 |
| **배치 파이프라인** | **X** | 유휴 구간이 짧고 반복적, 캐시 유실 치명적 |
| ML 학습 | X | 전체 리소스를 지속 사용 |
...
노드를 늘려봐야 I/O 대기 시간은 줄지 않습니다. **셔플/spill 데이터량 자체를 줄이는 것이 우선**입니다.

| 순서 | 튜닝 방법 | 효과 |
|:---:|----------|------|
| 1 | **파티션 수 늘리기** (`repartition(N)`) | 파티션당 데이터 크기 축소 → 메모리에 담기면 **spill(디스크 I/O) 제거** |
| 2 | **필터 선행 적용** | 셔플 전에 불필요 데이터 제거 → **네트워크 I/O 감소** |
| 3 | **broadcast join 적용** | 작은 테이블이면 셔플 자체 제거 → **네트워크 I/O 제거** |
| 4 | **캐시 전략 변경** | `DISK_ONLY` → `MEMORY_AND_DISK` → 반복 읽기 시 디스크 I/O 감소 |
| 5 | 위 방법으로 해결 안 되면 | 노드 수 줄이기 가능 (어차피 CPU 안 씀). 단, NVMe 대역폭 감소 주의 |

핵심: **노드를 더 사기 전에 I/O 자체를 없앨 수 있는지 먼저 확인**합니다. spill이 0이 되면 I/O bound가 아니라 CPU bound로 전환되어, 그때 노드 수를 판단합니다.

#### CPU bound일 때

```text
  → 노드 수 늘리기 또는 큰 인스턴스
  → 파티션 수도 코어 수에 맞게 조정 (코어 수의 2~3배)
```
...
```text
  ...
  → 이 포스트의 사례: repartition(800)으로 CPU 3% → 70%+ 개선
```
...
20대 유지 → 파티션 튜닝으로 활용률 3% → 70%+ 개선
 ↓
안정화 후 노드 줄이기 테스트:
  클러스터 인스턴스(중) × 8대 (64 cores, 15 TB+ NVMe)
  → 880GB 캐시 수용 가능
  → 코어 활용률 확인 후 추가 조정
```
...
| 9. Autoscaling | 배치 파이프라인은 고정 클러스터 | thrashing 방지, 캐시 유실 방지 |
| 10. 사이징 | 노드 수 조정 | 활용률 기반 점진적 축소 |
...
4. 배치 파이프라인에서는 autoscaling을 끄고 고정 클러스터를 사용하라. 배치 간 유휴 시간에 스케일다운되면 캐시 유실 + 리사이징 오버헤드로 오히려 느려진다.
