...
categories:
  - Spark
tags: [Spark, SparkSQL, Databricks, Join, Broadcast, AQE]
---
...
## 6. BroadcastHashJoin vs BroadcastNestedLoopJoin

---

`explain`에서 `BroadcastHashJoin`이 아니라 `BroadcastNestedLoopJoin`이 뜨는 경우가 있습니다. 둘 다 한쪽 테이블을 브로드캐스트하지만, 조인을 수행하는 방식이 다릅니다.

### 차이점

| 항목 | BroadcastHashJoin | BroadcastNestedLoopJoin |
|------|-------------------|------------------------|
| **조인 조건** | 등가 조건 (`=`) | 비등가 조건 (`<=`, `>`, `BETWEEN`, `!=`) 또는 조건 없음 |
| **내부 동작** | 작은 테이블로 해시맵 구축 → O(1) 룩업 | 모든 행을 1:1 비교 → O(N×M) |
| **성능** | 빠름 | 조합 수에 따라 느림 |
| **사용 조건** | 등가 조인 키 필수 | 조인 키 없어도 동작 |

### 언제 NestedLoopJoin이 뜨나?

**Range 조인**(범위 조건)을 쓸 때 가장 흔합니다.
...
등가 조건(`=`)이 없으면 해시맵을 만들 수 없어서, Spark는 작은 테이블의 모든 행과 큰 테이블의 모든 행을 1:1로 비교합니다. 작은 테이블이 1,000행이고 큰 테이블이 100만행이면 10억 번 비교합니다.

### 대응 방법

| 상황 | 해결책 |
|------|--------|
| 범위 조건만 있음 | Databricks에서는 `RANGE_JOIN` 힌트로 최적화 가능 |
| 등가 + 범위 혼합 | 등가 조건을 앞에 두면 `BroadcastHashJoin` 사용 후 범위 필터 적용 |
| 작은 테이블이 정말 작음 (수백 행) | NestedLoop이어도 성능 문제 없음 |
| 큰 테이블 × 큰 테이블 | 브로드캐스트 자체를 피하고 `SortMergeJoin` 사용 |

```sql
-- 등가 조건을 추가하면 BroadcastHashJoin으로 변경됨
FROM big_table b
JOIN /*+ BROADCAST */ small_table s
  ON b.category = s.category           -- 등가 조건 (해시 가능)
 AND b.timestamp >= s.start_time       -- 추가 필터로 처리
 AND b.timestamp <  s.end_time
```

핵심: `explain`에서 `BroadcastNestedLoopJoin`이 보이면, **조인 조건에 `=`이 있는지 확인**하세요. 등가 조건을 추가할 수 있다면 `BroadcastHashJoin`으로 바뀌어 성능이 크게 개선됩니다.
