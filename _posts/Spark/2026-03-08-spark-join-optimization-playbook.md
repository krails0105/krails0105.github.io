...
categories:
  - Spark
tags: [Spark, Join, Broadcast, Skew, Shuffle, Optimization]
---
...
스큐의 본질은 **같은 키가 한 파티션에 몰리는 것**입니다. Salting은 핫 키에 랜덤 접미사(salt)를 붙여서 하나의 키를 여러 키로 쪼개, 여러 파티션으로 분산시키는 기법입니다.

#### 원리

```
[Before - key="A"가 1,000만 건 몰림]
  Partition 0: key=A (10,000,000건) ← 이 파티션만 과부하
  Partition 1: key=B (1,000건)
  Partition 2: key=C (2,000건)

[After - salt 10개 적용]
  Partition 0: key=A_0 (1,000,000건)
  Partition 1: key=A_1 (1,000,000건)
  ...
  Partition 9: key=A_9 (1,000,000건)  ← 10개로 분산!
  Partition 10: key=B_0 (1,000건)
  ...
```
...
1. **왼쪽(큰 테이블)**: 각 행에 `0~9` 중 랜덤 숫자를 붙임 → `key=A` → `key=A_3`
2. **오른쪽(조인 대상)**: 모든 행을 salt 개수만큼 복제 → `key=A` → `A_0, A_1, ..., A_9`

왼쪽은 랜덤으로 하나만 붙이고, 오른쪽은 전부 복제하므로, 어떤 salt가 붙든 반드시 매칭됩니다.
...
| 장점 | 단점 |
|------|------|
| 스큐 완전 해소 | 오른쪽 테이블이 N배로 복제됨 (메모리/디스크 증가) |
| AQE 없는 환경에서도 동작 | 코드 복잡도 증가 |
| salt 수로 분산 정도 제어 가능 | salt 수가 크면 오른쪽 복제 비용 증가 |

**언제 쓰나**: AQE skew join이 해결 못하는 심한 스큐, 또는 Spark 3.0 미만 환경. AQE가 충분하면 salting 없이 해결되는 경우가 많으므로, AQE를 먼저 시도하고 안 될 때 salting을 적용합니다.
