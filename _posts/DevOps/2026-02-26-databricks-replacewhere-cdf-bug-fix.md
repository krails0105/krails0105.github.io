...
Bitcoin on-chain 데이터 파이프라인을 운영하면서 Delta Lake의 `replaceWhere`를 증분 적재 패턴으로 적극 활용하고 있습니다. 멱등성(idempotency)이 보장되고 Liquid Clustering과 궁합이 좋아서 채택한 방식인데, 실제 운영 중에 두 가지 예상 못한 버그를 만났습니다.
...
Bitcoin 블록 데이터 (S3)
  ...
  -> Bitcoin (network_block, market_indicator_block, utxo_statistics_block ...)
...
- `network_block`, `network_day`, `network_hour`
- `market_indicator_block`
- `utxo_statistics_block`, `utxo_age_distribution_block`
- 기타 5개 테이블
...
`validate_job_args.ipynb`의 `get_rollup_job_args()`에서 `start_ts`를 window 경계(boundary)에 맞게 truncate하도록 수정했습니다.

```python
# validate_job_args.py (수정 후)
def get_rollup_job_args():
    from datetime import datetime, timedelta, timezone
    from dateutil import parser

    end_ts = parser.isoparse(dbutils.widgets.get("end_ts")).replace(tzinfo=timezone.utc)
    window = dbutils.widgets.get("window")
    assert window in ["hour", "day"]

    # start_ts: widget으로 직접 지정 가능, 없으면 end_ts 기준 자동 계산
    all_widgets = dict(dbutils.widgets.getAll())
    _start_ts_raw = all_widgets.get("start_ts", "").strip()
    if _start_ts_raw:
        start_ts = parser.isoparse(_start_ts_raw).replace(tzinfo=timezone.utc)
    else:
        _time_delta = timedelta(hours=3) if window == "hour" else timedelta(days=2)
        start_ts = end_ts - _time_delta

    # 핵심 수정: replaceWhere 하한과 date_trunc 결과를 일치시키기 위해
    # start_ts를 window 경계(day=00:00:00, hour=XX:00:00)로 truncate
    if window == "day":
    ...
    return {"start_ts": start_ts, "end_ts": end_ts, "window": window}
```
...
`validate_job_args`는 공용 유틸이므로, 이 한 곳의 수정으로 `network_rollup`, `market_rollup` 등 모든 rollup 노트북에 일괄 적용됩니다.

추가로 `start_ts` widget을 수동으로 지정할 수 있는 경로도 함께 넣었습니다. 수동 지정된 `start_ts`도 동일하게 truncate를 거치므로, backfill 시에도 같은 문제가 재발하지 않습니다.
...
- `market_rollup`, `network_rollup` Databricks 실제 테스트 (history init 재실행 후)
- CDF ingestion PostgreSQL 수치 정합성 검증 (buggy 버전으로 적재된 데이터의 재처리 필요 여부 확인)
- `utxo_base_init.ipynb`에서 datetime JOIN 잔존 여부 확인 ([bug #8](/todo) 패턴)
...
- `cdf_module.py` -- CDF dedup 로직 (수정 대상)
- `validate_job_args.py` -- rollup job args 유틸 (수정 대상)
