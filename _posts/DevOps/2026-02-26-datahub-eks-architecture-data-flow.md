...
`datahub ingest -c recipes/databricks.yml`을 실행하면 Unity Catalog를 스캔해서 테이블 스키마, 설명, 소유자 등을 등록합니다. `{analytics_catalog}.bitcoin.*` 179개 테이블이 대상이었습니다.
...
source:
  ...
  config:
    project_ids:
      - {analytics-project}    # 테이블이 있는 프로젝트
      - {dw-project}    # dbt가 쿼리를 실행하는 프로젝트 (필수!)
    dataset_pattern:
      allow:
        - "{analytics-project}.bitcoin"
        - "{analytics-project}.raw_data.*"
        # ... 28개 데이터셋
    include_table_lineage: true
...
**왜 `{dw-project}`가 `project_ids`에 들어가야 하는가?**

DataHub BQ source는 lineage를 추출할 때 `INFORMATION_SCHEMA.JOBS` 테이블에서 최근 쿼리 로그를 읽습니다. 이 조회에는 `bigquery.jobs.listAll` 권한이 필요합니다. dbt는 `execution_project`(여기서는 `{dw-project}`)에서 쿼리를 실행하므로, 쿼리 로그가 이 프로젝트에 남습니다. `project_ids`에 이 프로젝트가 없으면 쿼리 로그를 읽지 못해 **table-to-table lineage가 0건**이 됩니다.

`{dw-project}`를 추가한 후 lineage 결과: **96건 BQ 내부 table-to-table lineage** 추출 성공.
...
flowchart LR
    subgraph Databricks
        DT["{analytics_catalog}\n.bitcoin.*\n179 tables"]
    end
    subgraph BQ["BigQuery"]
        BQT["{analytics-project}\n.bitcoin.*\n136 matched"]
    end
...
for table_name in matched_tables:
    upstream_urn = (
        ...
        f"{analytics_catalog}.bitcoin.{table_name},PROD)"
    )
    downstream_urn = (
        "urn:li:dataset:(urn:li:dataPlatform:bigquery,"
        f"{analytics-project}.bitcoin.{table_name},PROD)"
    )
