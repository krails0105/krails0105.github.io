...
tags: [BigQuery, Databricks, StorageWriteAPI, ETL, CostOptimization, PySpark, DeltaLake, GCS]
---

## 들어가며

Bitcoin on-chain ETL 파이프라인은 지금까지 `Delta Lake → PostgreSQL → BigQuery` 흐름으로 운영해왔다. PostgreSQL이 중간 버퍼 역할을 하는 구조인데, 실제로 PostgreSQL을 통해야 할 이유가 없다는 판단 아래 `Delta Lake → BigQuery` 직접 적재로 전환을 검토했다.
...
- Bitcoin on-chain 파생 테이블: 약 100개
- 전체 데이터: 약 50GB (블록 테이블 기준 ~940K rows x 30+ 컬럼)
- 월간 incremental: ~1-2GB (신규 블록 적재)
- Init (1회성): ~50GB write
...
table.clustering_fields = ["block_height"]  # 필요 시
...
정밀도가 중요한 Bitcoin 금액(satoshi 단위) 컬럼이라면 `BIGNUMERIC`을, 파생 지표(MVRV, SOPR 등)라면 `DOUBLE` 캐스팅이 간단하다.
...
cdf_df = spark.read.format("delta") \
    ...
    .table("tech_lakehouse_prod.bitcoin.indicator_block")
...
merge_query = f"""
MERGE `{BQ_TARGET}` T
USING `{BQ_STAGING}` S ON T.block_height = S.block_height
...
Bitcoin on-chain 데이터처럼 블록 단위 incremental이 소량(수십 MB/회)이고 init이 수십 GB 수준인 경우, **전량 `direct` 방식이 비용 효율적**이다. 2TB 무료 구간을 초과할 일이 거의 없기 때문이다.
...
**BQ 쿼리 비용이 진짜 변수**: Storage Write API 비용은 이 규모에서 사실상 무료지만, BQ를 API 서빙 레이어로 쓰기 시작하면 스캔 비용이 급증할 수 있다. 파티션(예: `DATE(timestamp)`)과 클러스터링(예: `block_height`) 설정을 테이블 생성 시점부터 잡아두는 것이 중요하다.
...
- BQ 파티션/클러스터링 설정 자동화 (테이블 생성 시 DDL에 포함)
- Storage Write API quota 모니터링 대시보드 연결
- 쿼리 비용 추적을 위한 `INFORMATION_SCHEMA.JOBS` 기반 리포트
- `indirect` 방식 fallback 구현 (quota 초과 대비)
