...
UniForm은 데이터 파일을 재작성하지 않는다. Databricks 공식 문서에 따르면 "without rewriting data, a single copy of the data files serves multiple formats"이다. 그러나 Delta write 후 Iceberg 메타데이터(snapshot)를 비동기로 생성할 때, **새 snapshot이 전체 테이블 상태를 포함하는 manifest를 만든다.** StarRocks는 이전 snapshot과 비교하여 변경된 파티션을 감지해야 하는데, UniForm이 생성한 Iceberg snapshot에서는 모든 파티션이 "변경됨"으로 인식되는 것으로 보인다.

이것이 핵심이다. UniForm의 Iceberg 메타데이터 생성 방식이 StarRocks의 파티션 변경 감지 메커니즘과 호환되지 않는 것이다. 따라서 **진정한 incremental refresh는 Iceberg 네이티브 테이블(Iceberg로 직접 write)이어야 가능**할 가능성이 높다.
...
3. 그러나 UniForm이 생성한 Iceberg snapshot에서 StarRocks가 변경된 파티션을 식별하지 못해 **사실상 풀 refresh**가 된다.
