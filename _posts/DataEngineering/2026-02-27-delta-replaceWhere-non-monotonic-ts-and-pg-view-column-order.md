...
이번 글에서는 Bitcoin on-chain 데이터 ETL 파이프라인에서 발견한 두 가지 버그를 다룹니다. 두 버그 모두 근본 원인은 동일합니다 -- 검증하지 않은 가정에 의존한 코드입니다.

| # | 이슈 | 암묵적 가정 | 실제 |
|---|------|-----------|------|
| Bug #15 | Delta `replaceWhere` 범위 위반 | block_height 순서 = datetime 순서 | Bitcoin 블록 타임스탬프는 비단조(non-monotonic) |
...
- **Bitcoin 블록체인** (도메인 배경): `block_height`(블록 번호)는 단조 증가하지만, `datetime`(블록 타임스탬프)은 단조 증가를 보장하지 않음
...
Bitcoin 블록 타임스탬프는 **단조 증가를 보장하지 않습니다.** Bitcoin 프로토콜의 타임스탬프 유효성 검사는 직전 11개 블록의 중앙값(Median Time Past)보다 크기만 하면 통과하기 때문에, `block_height`가 높아도 `datetime`이 이전 블록보다 이른 경우가 실제로 존재합니다.
...
price_df = spark.table("price_block") \
    .where(f"datetime >= '{start_ts}' AND datetime < '{end_ts}'")

# 2단계: 가격 데이터에서 block_height 범위 추출 (예: 884000 ~ 884020)
block_min = price_df.agg(F.min("block_height")).collect()[0][0]
block_max = price_df.agg(F.max("block_height")).collect()[0][0]

# 3단계: network_block을 block_height 범위로 필터링 후 시간 단위 집계
source_df = spark.table("network_block") \
    .where(f"block_height BETWEEN {block_min} AND {block_max}") \
...
source_df.write.format("delta") \
    ...
    .mode("overwrite") \
    .saveAsTable(network_table)
```

핵심 문제는 **1단계에서 datetime 기준으로 구한 block_height 범위를, 3단계에서 block_height 기준으로 다시 필터링**하는 부분입니다. datetime -> block_height -> datetime 변환 과정에서 타임스탬프 역전 row가 포함됩니다.

```
block_height 884001 -> datetime 00:58:00  (타임스탬프 역전)
date_trunc('hour', 00:58:00) -> 00:00:00
replaceWhere 조건: datetime >= 01:00:00   <-- 위반!
```

`block_height 884001`은 block_height 범위(`884000~884020`) 안에 있지만, 해당 블록의 datetime(`00:58:00`)은 시간 단위로 truncate하면 `00:00:00`이 됩니다. 이 값은 replaceWhere 하한인 `01:00:00`을 위반합니다.
...
`market_rollup.ipynb`은 MERGE 방식(`merge_sql_template`)을 사용하기 때문에 `DELTA_REPLACE_WHERE_MISMATCH` 에러가 직접 발생하지는 않습니다. 그러나 replaceWhere 범위 밖의 row가 MERGE source에 포함되면, 범위 경계를 넘은 불완전 집계가 upsert될 수 있습니다. 동일한 필터를 적용합니다.

```python
# market_rollup_job.py (수정 후)
...
> **Bitcoin(블록체인) 데이터에서 block_height와 datetime은 같은 방향으로 증가하지 않을 수 있습니다.**

- datetime 기반으로 block_height 범위를 구한 뒤, block_height 기반으로 데이터를 다시 필터링하면 **datetime 역전 row가 포함될 수 있습니다.**
...
```
InvalidTableDefinition: cannot change name of view column "block_height" to "entity_id"
```
...
|---|---|---|---|---|
| 1 | `block_height` | 1 | `entity_id` | 이름 불일치 -- 에러 |
| 2 | `entity_id` | 2 | `block_height` | 이름 불일치 -- 에러 |
| 3 | `value` | 3 | `value` | 일치 |

기존 view의 position 1(`block_height`)에 새 쿼리의 position 1(`entity_id`)을 매핑하려 하고, 이름이 달라 에러가 발생합니다.
...
```python
# view_change.py -- pq_view_change() 수정 후
...
| 가정 | 현실 | 결과 |
|------|------|------|
| block_height가 높으면 datetime도 최신일 것이다 | Bitcoin 타임스탬프는 비단조, 역전 가능 | `DELTA_REPLACE_WHERE_MISMATCH` |
...
1. **데이터 순서에 대한 가정을 코드에 넣지 말 것** -- 특히 외부 시스템(블록체인, API 등)에서 오는 데이터는 순서를 보장하지 않을 수 있습니다.
