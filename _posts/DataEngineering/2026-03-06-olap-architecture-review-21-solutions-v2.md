---
title: "[DataEngineering] 21개 OLAP 솔루션을 직접 검증한 데이터 파이프라인 서빙 레이어 교체기"
categories:
  - DataEngineering
tags:
  - OLAP
...
대규모 이벤트 데이터 ETL 파이프라인의 서빙 레이어를 교체하기 위해 21개 OLAP 솔루션을 체계적으로 평가했습니다. 이 글은 그 과정에서 발견한 기술적 사실들, 잘못된 정보를 공식 문서로 바로잡은 경험, 그리고 최종 후보 3개(ClickHouse, CelerData/StarRocks, Cube)의 실질적인 트레이드오프를 정리합니다.
...
- **ETL**: Databricks Asset Bundles + PySpark. Bronze/Silver/Derived 3-layer 구조
- **스토리지**: Delta Lake (Unity Catalog 관리, UniForm Iceberg 메타데이터 활성화 상태)
- **테이블 규모**: 다수의 테이블, 최대 대규모 (대형 테이블)
- **서빙 요구사항**: API 서빙 (수백 QPS) + 분석가 ad-hoc 쿼리
...
```
S3 (raw block JSON)
  └── Databricks Delta ETL (Bronze → Silver → Derived)
...
- 예상 비용: **$520-820/월** (m6g.xlarge 상당, 500GB 스토리지, ~수백 QPS 기준)
...
PROPERTIES(
    ...
    "databricks.catalog.name" = "{analytics_catalog}"
);
...
- **자체 DSL**(Data Model 정의)을 새로 학습해야 합니다. 다수의 테이블에 대한 모델 정의 작업이 초기 비용으로 발생합니다.
