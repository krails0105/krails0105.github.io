...
## 3. port-forward의 한계와 EKS CronJob
...
클러스터 내부에서 직접 실행하면 이 문제가 사라집니다. GMS 주소로 K8s Service DNS를 사용합니다.

```yaml
# EKS CronJob의 sink 설정
sink:
  type: datahub-rest
  config:
...
| 증상 | 원인 | 해결 |
|---|---|---|
| port-forward에서 sink failure 빈발 | 단일 TCP 프록시로 대량 MCP 전송 불안정 | EKS CronJob으로 클러스터 내부 실행 |
