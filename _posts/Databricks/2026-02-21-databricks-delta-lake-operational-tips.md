---
title: "[Databricks] Delta Lake 실전 운영 이슈 3가지 - IcebergCompatV3 DML 에러, 노트북 파라미터, DDL 컬럼 순서"
categories:
  - Databricks
tags:
  - Databricks
...
이 글은 Databricks에서 Delta Lake를 운영하면서 겪은 3가지 실전 문제와 해결법을 정리합니다.

| # | 이슈 | 핵심 원인 |
|---|------|----------|
| 1 | IcebergCompatV3 + UniForm DML 에러 | DBR 버전별 writer feature 지원 차이 |
| 2 | 노트북 간 파라미터 전달 | 4가지 방법의 장단점과 함정 |
| 3 | DDL 컬럼 순서 제어 | `set()`/`sorted()` 사용 시 순서 소실 |
