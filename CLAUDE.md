...
포스트에 **실제 프로젝트의 고유 정보가 노출되지 않도록** 반드시 일반화합니다.

- **데이터베이스/테이블명**: 프로덕션 DB 경로, 스키마명 → `my_database.table_name` 등 일반명으로 변경
- **컬럼명**: 도메인 특화 컬럼명 → `partition_key`, `sort_key`, `created_id` 등 역할 기반 일반명으로 변경
- **함수/변수명**: 프로젝트 고유 함수명 → `get_data()`, `process()` 등 일반명으로 변경
- **프로세스/메트릭명**: 내부 프로세스명, 메트릭명 → `MainProcess`, `metric_1` 등으로 변경
- **프로젝트명**: 실제 프로젝트명 대신 "대규모 데이터 처리 프로젝트" 등 일반적 표현 사용

> 기술적 설명에 필요한 **구조와 패턴은 유지**하되, 특정 조직/프로젝트를 식별할 수 있는 이름은 모두 일반화합니다.
